...
for i in range(epoch):
    for j,(image,_) in enumerate(train_loader):
        ...
        dis_loss = torch.sum(gan_loss_func(dis_fake,zeros_label))                 + torch.sum(gan_loss_func(dis_real,ones_label))                 + ratio *(torch.sum(cat_loss_func(onehot_fake,z_label_category))                 + torch.sum(contin_loss_func(contin_fake,z_label_contin)))
        ...
        gen_loss = torch.sum(gan_loss_func(dis_fake,ones_label))                  + ratio *(torch.sum(cat_loss_func(onehot_fake,z_label_category))                 + torch.sum(contin_loss_func(contin_fake,z_label_contin)))
