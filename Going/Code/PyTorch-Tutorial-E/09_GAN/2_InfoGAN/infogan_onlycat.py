...
for i in range(epoch):
    for j,(image,_) in enumerate(train_loader):
        ...
        for k in range(2):
            ...
            gen_loss = torch.sum(gan_loss_func(dis_fake,ones_label))                      + torch.sum(cat_loss_func(label_fake,z_variable))
        ...
        dis_loss = torch.sum(gan_loss_func(dis_fake,zeros_label))                 + torch.sum(gan_loss_func(dis_real,ones_label))
