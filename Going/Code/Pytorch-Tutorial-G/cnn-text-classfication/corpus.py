...
class Corpus(object):
    ...
    def save(self):
        ...
        print('train_src length - [{}]'.format(len(data['train']['src'])))
        print('valid_src length - [{}]'.format(len(data['valid']['src'])))

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='CNN Classification')
    parser.add_argument('--file-path', type=str, default="./data/cnn-text-classfication/data",
                        help='file path')
    parser.add_argument('--save-data', type=str, default="./data/cnn-text-classfication/corpus.pt",
